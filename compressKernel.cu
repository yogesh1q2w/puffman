...
__global__ void initOffsets(uint fileSize, uint *dfileContent,
    ...
    uint threadInput_idx = 0;
    uint *threadInput;
    __shared__ char sh_dictionary[256];
    __shared__ unsigned int shared_task_idx;

    sh_dictionary[threadIdx.x] = d_dictionary_codelens[threadIdx.x];
    ...
    while (task_idx < numTasks) {
        ...
        while (inputPosInThreadTask < PER_THREAD_PROC &&
            ...
            if ((inputPosInThreadTask & 3) == 0)
                input = threadInput[inputPosInThreadTask / 4];
            dbitOffsets[threadInput_idx + inputPosInThreadTask] =
                    sh_dictionary[GET_CHAR(input, inputPosInThreadTask & 3)];
            inputPosInThreadTask++;
        }
        if (threadIdx.x == 0) {
            shared_task_idx = atomicAdd(counter, 1);
